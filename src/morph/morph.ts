...
import { doAttributeMorph } from './attributes';
...
export function morph (oldNode: Element, newNode: Element, options?: any) {
  ...
  function doElementMorph (oldElement: Element, newElement: Element) {
    ...
    if (oldElement.getAttribute($.qs.$morph) !== 'children') {
      doAttributeMorph(oldElement, newElement);
    }
    ...
    while (curChild) {
      ...
      if (skipKeyedNodes && (key = getNodeKey(curChild))) {
        ...
        if (curChild.firstChild) {
          walkNodes(curChild as Element, skipKeyedNodes);
        }
      }
  ...
  if (morphedNode === newNode) {
    ...
    if (keyedRemovalList) {
      for (
        let i = 0
          , s = keyedRemovalList.length; i < s; i++) {
