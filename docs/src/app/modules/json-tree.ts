import session from "../references/session";
...
export const JSONTree = (function () {
  ...
  const utils = {
    ...
    /**
       ...
       */
    getType: function (val: any): string {
      if (val === null) {
        return 'null';
      }
    ...
    /**
       ...
       */
    forEachNode: function (obj: any[], func: (arg0: string, arg1: any, arg2: boolean) => void) {
      ...
      switch (type) {
        case 'array':
          ...
          obj.forEach(function (item: any, i: any) {
    ...
    inherits: (function () {
      ...
      return function (Child: { prototype: { constructor: any; }; }, Parent: { prototype: any; }) {
    ...
    /*
       ...
       */
    isValidRoot: function (jsonObj: any) {
    ...
    /**
       * Extends some object
       */
    extend: function (targetObj: { [x: string]: any; }, sourceObj: { [x: string]: any; hasOwnProperty: (arg0: string) => any; }) {
   ...
   *
   * Markup:
   * <li class="json-tree-node [json-tree-node_expanded]">
   *     <span class="json-tree-label-wrapper">
   *         <span class="json-tree-label">
   *             <span class="json-tree-expand-button" />
   *             "label"
   *         </span>
   *         :
   *     </span>
   *     <(div|span) class="json-tree-value json-tree-value-(object|array|boolean|null|number|string)">
   *         ...
   *     </(div|span)>
   * </li>
   *
   ...
   * @return {Node}
   */
  function Node (label: any, val: any, isLast: string): Node {
   ...
   *     <span class="json-tree-label-wrapper">
   *         <span class="json-tree-label">"age"</span>
   *         :
   *     </span>
   *     <span class="json-tree-value json-tree-value-(number|boolean|string|null)">25</span>
   *     ,
   * </li>
   *
   * @abstract
   ...
   */
  function _NodeSimple (label: any, val: any, isLast: any) {
    ...
    const template = function (label: string, val: string) {
      ...
      let tooltip: string = '';
      if(label in session) {
        tooltip = `data-tooltip="top-left" aria-label="${session[label]}"`
      }

      const str = `<span class="json-tree-label-wrapper"><span class="json-tree-label" ${tooltip}>${label}</span>:</span><span class="json-tree-value-wrapper"><span class="json-tree-value json-tree-value-${self.type}">${val}</span>${(!isLast ? ',' : '')}</span>`;
    ...
    el.classList.add('json-tree-node');
    el.innerHTML = template(label, val);

    self.el = el;

    labelEl = el.querySelector('.json-tree-label');

    labelEl.addEventListener('click', function (e: { altKey: any; shiftKey: any; }) {
      if (e.altKey) {
        self.toggleMarked();
        return;
      }
  ...
  _NodeSimple.prototype = {
    ...
    /**
       ...
       */
    mark: function () {
      this.el.classList.add('json-tree-node-marked');
    },
    ...
    /**
       ...
       */
    unmark: function () {
      this.el.classList.remove('json-tree-node-marked');
    },
    ...
    toggleMarked: function () {
      this.el.classList.toggle('json-tree-node-marked');
    },
    ...
    /**
       ...
       *                                (from node to root)
       */
    expandParent: function (isRecursive: any) {
      if (!this.parent) {
        return;
      }
    ...
    /**
       ...
       * @returns {string}
       */
    getJSONPath: function (isInDotNotation: any): string {
      if (this.isRoot) {
        return '$';
      }
   ...
   * @param isLast {boolean} - true if node is last in list of parent childNodes
   */
  function NodeBoolean (label: any, val: any, isLast: any) {
    this.type = 'boolean';
   ...
   * @param isLast {boolean} - true if node is last in list of parent childNodes
   */
  function NodeNumber (label: any, val: any, isLast: any) {
    this.type = 'number';
   ...
   * @param isLast {boolean} - true if node is last in list of parent childNodes
   */
  function NodeString (label: any, val: string, isLast: any) {
    this.type = 'string';
   ...
   * @param isLast {boolean} - true if node is last in list of parent childNodes
   */
  function NodeNull (label: any, val: any, isLast: any) {
    this.type = 'null';
   ...
   *
   * Markup:
   * <li class="json-tree-node json-tree-node_(object|array) [expanded]">
   *     <span class="json-tree-label-wrapper">
   *         <span class="json-tree-label">
   *             <span class="json-tree-expand-button" />
   *             "label"
   ...
   *         <b>{</b>
   *         <ul class="json-tree-child-nodes" />
   *         <b>}</b>
   *         ,
   *     </div>
   * </li>
   ...
   * @param isLast {boolean} - true if node is last in list of parent childNodes
   */
  function _NodeComplex (label: any, val: any, isLast: any) {
    ...
    const template = function (label: string, sym: string[]) {
      ...
      if (label !== null) {
        ...
        str = /* html */`
        <span class="json-tree-label-wrapper">
        <span class="json-tree-label" ${tooltip}>
    ...
    let childNodesUl: any;
    let labelEl: { addEventListener: (arg0: string, arg1: (e: any) => void, arg2: boolean) => void; };
    let moreContentEl: { addEventListener: (arg0: string, arg1: (e: any) => void, arg2: boolean) => void; };
    const childNodes = [];
    ...
    el.classList.add('json-tree-node');
    el.classList.add('json-tree-node-complex');
    el.innerHTML = template(label, self.sym);

    childNodesUl = el.querySelector('.json-tree-child-nodes');

    if (label !== null) {
      labelEl = el.querySelector('.json-tree-label');
      moreContentEl = el.querySelector('.json-tree-show-more');

      labelEl.addEventListener('click', function (e: { altKey: any; shiftKey: any; ctrlKey: any; metaKey: any; }) {
        if (e.altKey) {
          self.toggleMarked();
          return;
        }
      ...
      moreContentEl.addEventListener('click', function (e: { ctrlKey: any; metaKey: any; }) {
        self.toggle(e.ctrlKey || e.metaKey);
      }, false);
      ...
      el.classList.add('json-tree-node-expanded');
    }
    ...
    utils.forEachNode(val, function (label: any, node: any, isLast: any) {
      self.addChild(new Node(label, node, isLast));
    });

    self.isEmpty = !childNodes.length;
    if (self.isEmpty) {
      el.classList.add('json-tree-node-empty');
    }
  }
  ...
  utils.extend(_NodeComplex.prototype, {
    ...
    /*
       ...
       */
    addChild: function (child: { el: any; parent: any; }) {
    ...
    /*
       ...
       */
    expand: function (isRecursive: any) {
      if (this.isEmpty) {
        return;
      }

      if (!this.isRoot) {
        this.el.classList.add('json-tree-node-expanded');

      }

      if (isRecursive) {
        this.childNodes.forEach(function (item: { isComplex: any; expand: (arg0: any) => void; }, i: any) {
          if (item.isComplex) {
    ...
    /*
       ...
       */
    collapse: function (isRecursive: any) {
      if (this.isEmpty) {
        return;
      }

      if (!this.isRoot) {
        this.el.classList.remove('json-tree-node-expanded');

      }

      if (isRecursive) {
        this.childNodes.forEach(function (item: { isComplex: any; collapse: (arg0: any) => void; }, i: any) {
          if (item.isComplex) {
    ...
    /*
       ...
       */
    toggle: function (isRecursive: any) {
      if (this.isEmpty) {
        return;
      }

      this.el.classList.toggle('json-tree-node-expanded');

      if (isRecursive) {
        const isExpanded = this.el.classList.contains('json-tree-node-expanded');

        this.childNodes.forEach(function (item: { [x: string]: (arg0: any) => void; isComplex: any; }, i: any) {
          if (item.isComplex) {
      ...
      document.dispatchEvent(new CustomEvent('tree:toggle'))
    ...
    /**
       ...
       */
    findChildren: function (matcher: (arg0: any) => any, handler: (arg0: any) => void, isRecursive: any) {
      if (this.isEmpty) {
        return;
      }

      this.childNodes.forEach(function (item: { isComplex: any; findChildren: (arg0: any, arg1: any, arg2: any) => void; }, i: any) {
   ...
   * @param isLast {boolean} - true if node is last in list of siblings
   */
  function NodeObject (label: any, val: any, isLast: any) {
   ...
   * @param isLast {boolean} - true if node is last in list of siblings
   */
  function NodeArray (label: any, val: any, isLast: any) {
    this.sym = [ '[', ']' ];
    this.type = 'array';
   ...
   *
   * Markup:
   * <ul class="json-tree-tree clearfix">
   *     {Node}
   * </ul>
   *
   * @constructor
   ...
   */
  function Tree (jsonObj: any, domEl: any, tooltips: any = null) {
  ...
  Tree.prototype = {
    ...
    /**
       ...
       */
    appendTo: function (domEl: { appendChild: (arg0: any) => void; }) {
    ...
    /**
       ...
       */
    toSourceJSON: function (isPrettyPrinted: any): string {
      if (!isPrettyPrinted) {
  ...
  return {
    /**
       ...
       * @returns {Tree}
       */
    create: function (jsonObj: any, domEl: any): Tree {

      if(domEl) return new Tree(jsonObj, domEl);
